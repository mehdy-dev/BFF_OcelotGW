...
internal static class CallbackEndpoint
{
        ...
        [FromServices] IAuthenticationCallbackHandler authenticationCallbackHandler)
    {
        try
        {
           ...
           // var endpointName = context.Request.Path.TrimEnd("/login/callback");
            ...
            //var tokenResponse = await identityProvider.GetTokenAsync(@"https://localhost:8443/.auth/login/callback", code, codeVerifier, context.TraceIdentifier);
